...
- Otherwise:
  - By using the module `mime-types`, get the [MIME-type](https://intranet.hbtn.io/rltoken/0C5UFnqsJ6wsCiTmzyrqIw 'MIME-type') based on the `name` of the file
...
In real life, you can use a third party service like [Mailgun](https://intranet.hbtn.io/rltoken/qSytPo66mpfM0NHh_aOBcA 'Mailgun') to send real email. These API are slow, (sending via SMTP is worst!) and sending emails via a background job is important to optimize API endpoint.
